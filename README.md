...
<p align="center">
  The <strong>Kiro CLI edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside Kiro CLI</strong>. Part of the Gald3r framework
...
This is the **Kiro CLI distribution** of Gald3r. The [`kiro-cli/`](./kiro-cli/) folder is a
complete, ready-to-deploy gald3r setup tuned for Kiro CLI -- drop its contents into your
...
cp -r gald3r_platform_kiro_cli/kiro-cli/. /path/to/your/project/
```

Then open the project in Kiro CLI and type `@g-status` to confirm gald3r loaded.

> The `kiro-cli/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
