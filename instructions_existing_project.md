# Adding Gald3r to an Existing Project (Kiro CLI)

Add gald3r to a project that **already exists**, for use with **Kiro CLI**.

For a brand-new project, see [instructions_new_project.md](./instructions_new_project.md).

---

## Step 1 - Get the `kiro-cli/` payload

```bash
git clone --depth 1 https://github.com/wrm3/gald3r_platform_kiro_cli.git
```
...
**bash / macOS / Linux:**
```bash
cp -r gald3r_platform_kiro_cli/kiro-cli/. /path/to/your/existing/project/
```

**Windows PowerShell:**
```powershell
Copy-Item -Recurse -Force .\gald3r_platform_kiro_cli\kiro-cli\* C:\path\to\your\project\
```
...
Reload the project in Kiro CLI. gald3r auto-loads from its config.
